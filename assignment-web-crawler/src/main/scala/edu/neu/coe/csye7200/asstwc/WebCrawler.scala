...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[Try[URL]] = for (n <- ns \\ "a"; href <- n\"@href"; href_head <- href.head) yield createURL(Some(u), href_head.toString())// TO BE IMPLEMENTED
    ...
    // 9 points.
    for (link <- getURLContent(u); link_future <- MonadOps.asFuture(getLinks(link))) yield link_future // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    val w = for (us <- us2) yield MonadOps.sequence(wget(us))
    Future.sequence(w)
  }
